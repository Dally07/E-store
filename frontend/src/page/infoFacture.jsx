...
const InfoFacture = () => {
  ...
  // Ajoutez cette fonction à votre composant InfoFacture
const sendEmail = async () => {
  const templateParams = {
      to_name: client.nomCli,
      to_email: client.emailCli, 
      message: `
          Voici votre facture n°${paiement.idPaiement}
          Client: ${client.nomCli}
          Total: ${paiement.commande.total} MGA
          
          Détails des produits:
          ${paiement.commande.produits.map(produit => `
              Nom: ${produit.nom}
              Prix: ${produit.prix} MGA
              Quantité: ${produit.Commande_Produit.quantite}
          `).join('')}
      `
  };

  try {
      const response = await emailjs.send('service_y304sgh', 'template_5rxug5m', templateParams, 'fu5OdFn-nhDuLAY-O');
          ...
          <div className="p-4 mb-6 text-white">
            ...
            <div className="flex justify-between items-center mb-2">
              <div className="flex space-x-4">
                ...
                <button onClick={sendEmail}  className="flex items-center px-4 py-2 bg-gray-600 text-white rounded-lg"style={{backgroundColor: "#041122"}}>
            ...
            <div id='section-imprimer2' className="grid grid-cols-2 gap-4">
            {client ? ( 
                <div className=" p-4 rounded-lg" style={{backgroundColor: "#041122"}}>
                  ...
                  <p className="text-xl"> Adresse et Telephone</p>
                  <p className="text-sm">{client.adresseCli}</p>
